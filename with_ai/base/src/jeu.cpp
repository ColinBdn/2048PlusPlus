...
#include "useful.h"
#include "keyboard.h"
#include "includeForSimpleGA.h"
#include "includeForNeat.h"
...
//-------------------------- ia --------------------------
/**
 * Function to run a simple Genetic Algorithm (GA) for training a population.
 *
 * This function initializes a static population of size 100 and trains it using the GA method.
 * The trained population remains static across function calls.
 */
void simpleGA()
{
    static SimpleGA::Population population(100);
    population.train();
}


/**
 * Function to train a NEAT (NeuroEvolution of Augmenting Topologies) population for XOR problem.
 *
 * This function configures NEAT settings for training on the XOR problem.
 * It initializes a NEAT population, trains it, saves the best brain, and validates it.
 * It also allows interactive plotting and stopping of the training process.
 */
void xorTrain()
{
    setMainBuffer();
    Neat::Settings settings;
    settings.mutate_Bias = 0.1;
    settings.mutate_Bias_power = 0.1;
    settings.mutate_Weight = 0.8;
    settings.mutate_Weight_power = 0.1;
    settings.mutate_Weight_reset = 0.00;
    settings.mutate_Node = 0.03;
    settings.mutate_Connection = 0.05;
    settings.mutate_EnableConnection = 0.01;
    settings.mutate_DisableConnection = 0.000;
    settings.num_species_target = 10;
    settings.compat_mod = 0.0;
    settings.stagnation_treshold = 1000;
    settings.speciation_threshold = 3;

    Neat xorPop(150);
    xorPop.init(2, 1);
    xorPop.settings = settings;
    GlHelper::add(&Neat::plot, &xorPop, "xor plotting");
    xorPop.train(500, &xorTraining, &xorValidate, true);

    Brain best = xorPop.getBestWinner();
    best.writeToFile("xor_best.brain");
    best.printAsync();
    xorValidate(&best);
    std::cout << "press enter to quit training...";
    while (!Keyboard::isKeyPressed(VK_RETURN))
    {
        Sleep(10);
    }
    best.stopPrinting();
    GlHelper::remove("xor plotting", &xorPop);
}

/**
 * Function to train a NEAT population for single pole balancing problem.
 *
 * This function configures NEAT settings for training on the single pole balancing problem.
 * It initializes a NEAT population, trains it, saves the best brain, and validates it.
 * It also allows interactive plotting and stopping of the training process.
 */
void spbTrain()
{
    ...
    Neat::Settings settings;
    settings.mutate_Bias = 0.1;
    settings.mutate_Bias_power = 0.005;

    settings.mutate_Weight = 0.8;
    settings.mutate_Weight_power = 0.8;
    settings.mutate_Weight_reset = 0.01;
    settings.mutate_Weight_max = 8;

    settings.mutate_Node = 0.002;
    settings.mutate_Connection = 0.005;

    settings.mutate_EnableConnection = 0.001;
    settings.mutate_DisableConnection = 0.001;

    settings.num_species_target = 50;
    settings.compat_mod = 0.1;
    settings.stagnation_treshold = 20;
    settings.speciation_threshold = 3;


    Neat spbPop(500);
    spbPop.init(4, 1);
    spbPop.settings = settings;
    GlHelper::add(&Neat::plot, &spbPop, "spb plotting");
    spbPop.train(10000, &spbTraining, &spbValidate, true);

    Brain best = spbPop.getBestWinner();
    best.writeToFile("spb_best.brain");
    best.printAsync();
    spbValidate(&best);
    best.stopPrinting();
    GlHelper::remove("spb plotting", &spbPop);
}

void train2048impl(Brain* brain)
{
    Grille g;
    init(g, 4, 2048, 9);
    int jsp = 0;
    while (true)
    {
        ...
        std::vector<float> vec1d = Matrix::vec2dToVec1d(g.table);
        ...
        brain->forward(input_norm);


        std::vector<float> output = brain->getOutput();
        float max = 0;
        ...
        if (vides(g) > 0 && has_moved != -1)
            spawnRandom(g);
        else
            break;
    }
    if (g.score > 6000 && false)
    {
        afficheCustom(g);
        while (!Keyboard::isKeyPressed(VK_RETURN))
        {
    ...
    brain->fitness = g.score;
}

void validate2048(Brain* brain)
{
    clearConsole(SYSTEM_CLEAR);
    Grille g;
    init(g, 4, 2048, 9);
...
void train2048()
{
    ...
    settings.mutate_Weight_power = 0.1;
    settings.mutate_Weight_reset = 0.01;
    settings.mutate_Node = 0.01;
    settings.mutate_Connection = 0.03;
    settings.mutate_EnableConnection = 0.005;
    settings.mutate_DisableConnection = 0.005;
    settings.num_species_target = 50;
    settings.compat_mod = 0.01;
    settings.stagnation_treshold = 20;
    settings.speciation_threshold = 6;

    Neat pop2048(500);
    pop2048.settings = settings;
    pop2048.init(16, 4);
    GlHelper::add(&Neat::plot, &pop2048, "2048 plotting");
    pop2048.train(10000, &train2048impl, &validate2048, true);

    Brain best = pop2048.getBestWinner();
    best.writeToFile("2048_best.brain");
    best.printAsync();
    validate2048(&best);
    best.stopPrinting();
    GlHelper::remove("2048 plotting", &pop2048);
}

/**
 * Implementation of the AI menu interface for controlling AI training processes.
 *
 * This function displays an ImGui-based menu for controlling different AI training processes,
 * including Simple Genetic Algorithm (GA) and NEAT (NeuroEvolution of Augmenting Topologies).
 * Users can start training for XOR and single pole balancing problems, stop training by pressing
 * the Escape key, and view the resulting brain files. The menu also allows users to select brain
 * files for validation and printing.
 *
 * @param p_open Pointer to a boolean flag indicating whether the AI menu is open or closed.
 * @param status Pointer to a string representing the current status or action triggered from the menu.
 * @param brainPath Pointer to a string representing the path of the selected brain file.
 */
void aiMenuImpl(bool* p_open, std::string* status, std::string* brainPath)
{
    //-------------------------------------------------------
    ...
    ImGui::Begin("ai menu", p_open, ImGuiWindowFlags_NoScrollbar);
    if (*p_open == false)
    {
        *status = "quit";
    }
    ...
    std::vector<std::string> brainFileNames;
    std::filesystem::path currentDir = std::filesystem::current_path();
    for (const auto& entry : std::filesystem::directory_iterator(currentDir)) {
        if (entry.path().extension() == ".brain") {
            brainFileNames.push_back(entry.path().filename().string());
        }
    }
    if (brainFileNames.size() == 0)
    {
        brainFileNames.push_back("no saved brain");
    }

    //-------------------------------------------------------


    ImGui::SeparatorText("Simple GA");
    if (ImGui::Button("start 2048 training##ga"))
    {
        *status = "simpleGaTrain";
    }
    ImGui::NewLine();

    ImGui::SeparatorText("Neat");
    if (ImGui::Button("start xor training"))
    {
        *status = "xorTrain";
    }

    ImGui::SameLine();
    ImGui::Text("you can stop training by pressing escape");
    ImGui::Text("the resulting brain of the training is automatically as 'xor_best.brain'");
    static int xor_selectedBrain = 0;
    ImGui::SetNextItemWidth(200);
    ImGui::Combo("Select a file##0", &xor_selectedBrain, [](void* data, int n) -> const char* {
        auto& fileNames = *static_cast<std::vector<std::string>*>(data);
        ...
        }, &brainFileNames, brainFileNames.size());
    static bool xorTrying = false;
    static Brain* xorBrain = nullptr;
    ImGui::Checkbox("execute the xor test and print brain##0", &xorTrying);
    if (xorTrying)
    {
        if (xorBrain == nullptr)
        {
            xorBrain = new Brain(Brain::readFromFile(brainFileNames[xor_selectedBrain]));
            *brainPath = brainFileNames[xor_selectedBrain];
            *status = "xorValidate";
        }
        xorBrain->print();
    }
    else if (xorBrain != nullptr)
    {
        delete xorBrain;
        xorBrain = nullptr;
    }
    ...
    if (ImGui::Button("start single pole balancing training"))
    {
        *status = "spbTrain";
    }
    ...
    ImGui::SameLine();
    ImGui::Text("you can stop training by pressing escape");
    ImGui::Text("the resulting brain of the training is automatically as 'spb_best.brain'");
    static int spb_selectedBrain = 0;
    ImGui::SetNextItemWidth(200);
    ImGui::Combo("Select a file##1", &spb_selectedBrain, [](void* data, int n) -> const char* {
        auto& fileNames = *static_cast<std::vector<std::string>*>(data);
        if (n >= 0 && n < fileNames.size()) {
            return fileNames[n].c_str();
        }
    ...
    static bool spbTrying = false;
    static Brain* spbBrain = nullptr;
    ImGui::Checkbox("execute the single pole balancing test and print brain##1", &spbTrying);
    if (spbTrying)
    {
        if (spbBrain == nullptr)
        {
            spbBrain = new Brain(Brain::readFromFile(brainFileNames[spb_selectedBrain]));
            *brainPath = brainFileNames[spb_selectedBrain];
            *status = "spbValidate";
        }
        spbBrain->print();
    }
    else if (spbBrain != nullptr)
    {
        delete spbBrain;
        spbBrain = nullptr;
    }
    ...
    if (ImGui::Button("start 2048 training##neat"))
    {
        *status = "2048Train";
    }
    ...
    ImGui::SameLine();
    ImGui::Text("you can stop training by pressing escape");
    ImGui::Text("the resulting brain of the training is automatically as '2048_best.brain'");
    static int selectedBrain_2048 = 0;
    ImGui::SetNextItemWidth(200);
    ImGui::Combo("Select a file##2", &selectedBrain_2048, [](void* data, int n) -> const char* {
        auto& fileNames = *static_cast<std::vector<std::string>*>(data);
        if (n >= 0 && n < fileNames.size()) {
            return fileNames[n].c_str();
        }
    ...
    static bool trying2048 = false;
    static Brain* brain2048 = nullptr;
    ImGui::Checkbox("execute the 2048 test and print brain##2", &trying2048);
    if (trying2048)
    {
        if (brain2048 == nullptr)
        {
            brain2048 = new Brain(Brain::readFromFile(brainFileNames[selectedBrain_2048]));
            *brainPath = brainFileNames[selectedBrain_2048];
            *status = "2048Validate";
        }
        brain2048->print();
    }
 ...
 *
 * This function initializes the AI menu interface and controls the flow of AI training and evaluation.
 * It allows executing training for XOR and single pole balancing problems and handles user interactions.
 */
void startGameAI()
{
    std::string status = "none"; // -1: rien, 0: quit, 1: xor, 2:sspb
    std::string brainPath;
    GlHelper::add(&aiMenuImpl, "aiMenu", &status, &brainPath);
    while (status != "quit")
    {
        if (status == "xorTrain")
        {
        ...
        else if (status == "xorValidate")
        {
            clearConsole(SYSTEM_CLEAR);
            Brain brain = Brain::readFromFile(brainPath);
            xorValidate(&brain);
            status = "none";
        }
        else if (status == "spbTrain")
        {
            clearConsole(SYSTEM_CLEAR);
            spbTrain();
            clearConsole(SYSTEM_CLEAR);
...
int mainMenu()
{
    ...
    if (_interactive)
    {
        ...
        while (!has_chosed)
        {
            ...
            printCentered("- Ai -", 0, posIndexY, root); posIndexY++;
            ...
            while (previousIndex == index && !has_chosed)
            {
                ...
                if (Keyboard::isKeyPressed(VK_DOWN) && index < 3) {
                    index++;
                }
                ...
                if (Keyboard::isKeyPressed(VK_RETURN) && index >= 0 && index <= 3) {
                    if (index == 0) choice = '1';
                    else if (index == 1) choice = '2';
                    else if (index == 2) choice = '3';
                    else choice = 'q';
                    has_chosed = true;
                }
            }
